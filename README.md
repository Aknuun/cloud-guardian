...
- ✉️ **ایمیل سازمانی** (داخل ✨ فیچرهای جدید): دریافت ایمیل‌های دامنه‌ها در ربات — هر دامنه با یک دکمه وصل می‌شود (چند دامنه همزمان)، متن ایمیل‌ها در صندوق نمایش داده می‌شود و لینک‌های داخل ایمیل قابل کلیک‌اند. موقع وصل دامنه می‌توانی «📩 فوروارد به ایمیل مقصد» (فورواردینگ روتیـنگ کلادفلر) را انتخاب کنی تا ایمیل‌های آینده مستقیم به آدرس مقصد (مثلاً جیمیل) بروند؛ مقصد باید اول با لینک داخل ایمیل تأیید شود.
- 📊 **تله‌متری ناشناس**: هر نصب روزی یک پینگ ناشناس (آیدی تصادفی + نسخه، بدون هیچ دیتای شخصی) می‌فرستد؛ آمار نصب‌های فعال فقط در تنظیمات ربات اصلی سازنده دیده می‌شود. غیرفعال‌سازی: `python3 deploy-tool.py set-kv telemetry_off 1`.
- 📢 **پیام همگانی** (داخل تنظیمات، فقط ادمین اصلی): پیام به همه ادمین‌های همه نصب‌ها می‌رسد (با دکمه پاسخ/تایید).
- 📊 **ترافیک سایتها** (داخل ✨ فیچرهای جدید): مرکز ترافیک DNS همهٔ دامنه‌ها — همهٔ ساب‌ها حتی صفرها دیده می‌شوند، با صفحه‌بندی و حذف ساب‌های بدون ترافیک.
- ⚖️ **لود بالانسر** (داخل ✨ فیچرهای جدید): مدیریت لود بالانسر — انتخاب دامنه، دیدن همهٔ ساب‌ها با چند رکورد هم‌نام، ساخت جدید، تنظیم وزن/فعال/غیرفعال آی‌پی‌ها.
