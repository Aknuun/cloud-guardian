...
نسخهٔ جدید از گیتهاب که منتشر شود، ربات **خودش را** آپدیت میکند (چک هر ۱۰ دقیقه، حداقل فاصلهٔ یک ساعت). شرط: bindingهای `WORKER_ACCOUNT_ID` و `WORKER_NAME` (در نصب خودکار اضافه میشوند) و توکن CF با دسترسی `Workers Scripts Edit`. مدیر پیام «🔄 آپدیت خودکار: vX ← vY» میگیرد.
