...
const RELEASE_NOTES = {
  "1.5.16": [
    "🔑 توکن فقط ۸ مجوز (۴ اجباری + ۴ اختیاری): بدون اجباری‌ها نصب جلو نمی‌رود، بدون اختیاری‌ها با هشدار نصب موفق می‌شود",
  ],
  "1.5.15": [
  ...
  "1.5.10": [
    "🛠 وان‌لاینر هوشمند شد: اجرای بدون آرگومان مستقیم آپدیت/نصب می‌کند و منو نشان نمی‌دهد (منو فقط برای فایل لوکال؛ دستور menu هم اضافه شد)",
  ],
  "1.5.9": [
    "🛠 اسکریپت نصب: نیازمندی‌ها (curl/python3) همان اول خودکار نصب می‌شوند بدون پرسیدن + اگر نصب قبلی هست وان‌لاینر مستقیم آپدیت می‌کند (فول‌اینستال اجباری نیست؛ نصب کامل فقط با --force)",
  ],
  "1.5.8": [
  ...
  "1.3.1": [
    ...
    "📤 خروجی Termius (CSV): فایل سرورها با قالب ایمپورت Termius + رفع باگ‌های بازبینی کامل (نصب نود، bulk_main، امنیت contact و...)",
  ],
  "1.3.0": [
    "📥 وارد کردن سرورها از هتزنر/لینود/آروان به لیست سرورها (با رمز پیش‌فرض اختیاری) ـ 📤 خروجی CSV سرورها با قالب ایمپورت Termius",
    "🗂 حالت عملیات گروهی در صفحهٔ نتایج جست‌وجوی آیپی (انتخاب چندتایی + حذف/تغییر مقدار/منتخب‌ها، چندزونه)",
    "🐞 رفع باگ بحرانی نصب نود (آرگومان جاافتاده) ـ رفع تداخل عملیات گروهی bulk_main ـ گارد نشست‌های منقضی در انتخاب/حذف گروهی",
    "🔒 امنیت: اعتبارسنجی تحویل پاسخ سازنده، ریت‌لیمیت تماس، مقصد پاسخ سفت‌وسخت‌تر، ماسک رمزهای SSH، هشدار رلهٔ http",
    "✅ تأیید حذف برای حذف سرور از لیست، اسنپ‌شات آروان و لود بالانسر ـ رفع اندپوینت حالت توسعه ـ انصراف گروهی بدون پاک‌شدن تیک‌ها",
  ],
  "1.2.0": [
    "🧹 استانداردسازی رابط کاربری: اصلاح متن تنظیمات، حذف دکمه‌های تکراری رله از راهنما، خلاصه‌سازی راهنما و دستور /myid در منوی ☰",
    "☁️ کلودفلر: مخفی‌شدن فلش صفحه‌بندی اضافی، «بازگشت»→«خانه» و دکمهٔ «👤 اکانت‌ها»",
    "📋 رکوردها: حذف قرمز و ایموجی 🛰 برای Proxy ـ ➕ افزودن رکورد: نوع سه‌ستونه + دکمهٔ «روشن‌کردن Proxy» بعد از ساخت",
    "🖥 مانیتورها: منوی یکدست (/mons هماهنگ شد)، ایموجی 📡 برای نود و «⏰ یادآورها» ـ 🖥 سرورها: حذف قرمز و افزودن گروهی 📥",
    "🌐 رله: انصراف‌ها به صفحهٔ رله برمی‌گردند ـ ✅ تأییدهای حذف یکدست («بله، حذف کن»)؛ کارهای بی‌خطر همچنان بدون تأیید",
  ],
  "1.0.31": [
...
function safeReplyUrl(u) {
  ...
  try {
    const uu = new URL(s);
    if (uu.username || uu.password || uu.port) return "";
    host = uu.hostname.toLowerCase();
  } catch (e) {
    return "";
  }
  if (!host || host.length > 253) return "";
  // بدون IPv6/براکت، بدون localhost/داخلی، و حتماً دارای حرف (نه IP عددی/هگز/دهدهی)
  if (host.includes(":") || host.includes("[") || host.includes("]")) return "";
  if (!/[a-z]/.test(host)) return "";
  if (host === "localhost" || host.endsWith(".localhost") || host.endsWith(".local") || host.endsWith(".internal") || host.endsWith(".invalid")) return "";
  return s.replace(/\/+$/, "");
}
// ============================================================
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST" && url.pathname === "/contact-reply") {
      ...
      let th = null;
      try {
        th = tid ? await kv.get(`ct:${tid}`, "json") : null;
      } catch (e) {}
      if (!th || String(th.chat_id || "") !== String(c.chat_id || "")) return ok();
      if (chat) {
        const kb = tid
          ? [[
...
async function quotaResume(env, botToken) {
  ...
  // روشن‌کردن دستی باید خاموشی موقت کرون را هم لغو کند (وگرنه تا پایان مهلت فقط گارد اجرا می‌شود)
  try {
    const cfg = await getQuotaCfg(kv, env);
    cfg.cronPausedUntil = 0;
...
async function getZoneById(zoneId, accIndex, accounts) {
  const acc = accounts && accounts[accIndex];
  if (!acc) return null;
  const res = await fetch(`${CF_API}/zones/${zoneId}`, { headers: hdr(acc.token), signal: withTimeout() });
  const data = await res.json();
  return data.success ? { ...data.result, _acc: accIndex, _accName: acc.name } : null;
}
...
function cfErrText(data) {
  ...
  if (isPermErrors(data)) {
    out =
      ...
      "این بخش به مجوز بیشتری نیاز دارد.\n" +
      "چون توکن قابل ویرایش نیست: توکن جدید بساز (با همهٔ دسترسی‌های قبلی + دسترسی جدید)، بعد توی ربات جایگزین کن:\n" +
...
async function renderLbZonesList(page, filter, edit, kv, accounts, env) {
  ...
  title += "\n\nروی دامنه کلیک کن تا ساب‌هاش رو ببینی.\n\n• «بالانسر۱/۲/...» وزن هر آی‌پی است (سهمش از ترافیک)\n• «🛟 زاپاس» آی‌پی/CNAME رزرو است؛ اگر اصلی فیلتر شد با یک دکمه جایگزین می‌شود";
  await edit(title, kb);
}
...
async function srvAuthKeyboard(kv) {
  ...
  return pws.map((p, i) => [{ text: `🔑 رمز ${i + 1} (${maskPw(p)})`, callback_data: `srvauthpw:${i}` }]);
}
...
// رمزهای SSH فقط ماسک‌شده نمایش داده می‌شوند (نه متن کامل در دکمه/تاریخچهٔ چت)
function maskPw(p) {
  const s = String(p || "");
...
🔐 ${authLabel}` +
  ...
  if (type === "srv_edit") {
    ...
    if (f === "name") {
      ...
      if (!u) return send("❌ نام کاربر خالی است. دوباره بفرستید:");
      s.user = u === "-" ? "root" : u.slice(0, 32);
    } else if (f === "auth") {
      if (txt !== "-") {
  ...
  if (type === "rem_when") {
    ...
    // اعتبارسنجی رفت‌وبرگشت (مثلاً اسفند ۳۰ فقط در سال کبیسه درست است)
    let okDate = false;
    try {
      const g = toGregorian(jy, jm, jd);
      const b = toJalaali(g.gy, g.gm, g.gd);
      okDate = b.jy === jy && b.jm === jm && b.jd === jd;
    } catch (e) {}
    if (!okDate) {
      await send("❌ این روز در این ماه/سال وجود ندارد. دوباره بفرستید:");
      return;
    }
    let at;
    try {
...
async function handleCallback(cb, botToken, adminId, kv, env) {
  ...
  try {
    ...
    if (data === "menu") {
      ...
      const pages = Math.max(1, Math.ceil(items.length / CZR_PAGE_SIZE));
      if (page < 0) page = 0;
      if (page >= pages) page = pages - 1;
      ...
      let cur = "";
      try {
        const gr = await fetch(`${CF_API}/zones/${session.zone_id}/settings/development_mode`, {
          headers: hdr(accounts[session.acc].token),
          signal: withTimeout(),
        });
        const gd = await gr.json();
        cur = gd && gd.success && gd.result ? String(gd.result.value) : "";
      } catch (e) {}
      const newMode = cur === "on" ? "off" : "on";
      ...
      const records = await getRecords(zone, accounts, kv);
      await renderRecords(zone, records, token, page, edit, undefined, session.zback);
    } else if (data.startsWith("selback:")) {
      ...
      if (!zone) return edit("❌ دامنه پیدا نشد.");
      const records = await getRecords(zone, accounts, kv);
      await renderRecords(zone, records, token, Number(selState.page) || 0, edit, selState.ids || [], session.zback);
      ...
      await edit(`⚠️ ${selState.ids.length} رکورد حذف شود؟`, [
        [{ text: "✅ بله، حذف کن", callback_data: `bulkdely:${token}` }, { text: "❌ انصراف", callback_data: `selback:${token}` }],
      ]);
      ...
      await edit("✏️ مقدار جدید را برای رکوردهای انتخاب‌شده بفرستید:", [
        [{ text: "⬅️ انصراف", callback_data: `selback:${token}` }],
      ]);
      ...
      // حذف از لیست سرورها (با تأیید، مثل حذف از صفحهٔ سرور) — بر اساس شناسهٔ یکتا
  ...
  if (m) {
    ...
    for (const sn of arr.slice(0, 10)) {
      ...
      kb.push([
        ...
        { text: "🗑", callback_data: `arvsnapdel:${m[1]}:${m[2]}:${sn.id || ""}` },
      ]);
    }
...
async function showBulkSelect(kv, accounts, edit, token, zone, page, selected) {
  ...
  if (selSet.size > 0) {
    kb.push([
      ...
      { text: "🗑 حذف انتخاب‌ها", callback_data: `bdel:${token}` },
      { text: "✏️ تغییر مقدار", callback_data: `bedit:${token}` },
    ]);
    // btype: تغییر نوع گروهی | bulkdne: لغو انتخاب
    kb.push([{ text: "🔄 تغییر نوع", callback_data: `btype:${token}` }, { text: "❌ لغو انتخاب", callback_data: `bulkdne:${token}` }]);
  } else {
